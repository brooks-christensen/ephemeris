...
RUN_OUTPUT_DIR="${RUN_OUTPUT_DIR:-${OUTPUT_BASE}/two_body_tangent_duration_runs}"
LADDER_CSV="${LADDER_CSV:-${OUTPUT_BASE}/two_body_tangent_duration_ladder.csv}"
SCALING_CSV="${SCALING_CSV:-${OUTPUT_BASE}/two_body_tangent_duration_scaling_summary.csv}"
SCALING_JSON="${SCALING_JSON:-${OUTPUT_BASE}/two_body_tangent_duration_scaling_summary.json}"
NO_RENORM_CSV="${NO_RENORM_CSV:-${OUTPUT_BASE}/tangent_no_renorm_duration_summary.csv}"
RECORD_EVERY_YEARS="${RECORD_EVERY_YEARS:-100}"
MAX_CASES="${MAX_CASES:-}"
SKIP_EXISTING="${SKIP_EXISTING:-1}"
INCLUDE_MERCURY_10000="${INCLUDE_MERCURY_10000:-0}"
RUN_NO_RENORM="${RUN_NO_RENORM:-1}"

mkdir -p "${RUN_OUTPUT_DIR}"

cd "${PROJECT_ROOT}"

export PYTHON_BIN KERNEL_PATH OUTPUT_BASE RUN_OUTPUT_DIR LADDER_CSV SCALING_CSV SCALING_JSON NO_RENORM_CSV
export RECORD_EVERY_YEARS MAX_CASES SKIP_EXISTING INCLUDE_MERCURY_10000 RUN_NO_RENORM
...
no_renorm_csv = Path(os.environ["NO_RENORM_CSV"])
...
include_mercury_10000 = os.environ.get("INCLUDE_MERCURY_10000", "0") == "1"
run_no_renorm = os.environ.get("RUN_NO_RENORM", "1") != "0"

base_configs = {
    "two_body_jupiter": {
        "body": "jupiter",
        "steps": [4.0, 2.0, 1.0],
        "renorms": [1.0, 5.0, 10.0],
    ...
    "two_body_mercury": {
        ...
        "renorms": [0.05, 0.1, 0.25],
        "perturbations": [1.0, 1000.0],
        "durations": [100.0, 300.0, 1000.0, 3000.0],
    },
}
if include_mercury_10000:
    base_configs["two_body_mercury"]["durations"].append(10000.0)
...
no_renorm_fields = [
    ...
    "duration_years",
    "perturbation_m",
    "log_norm_vs_time_slope",
    "log_norm_vs_time_r_squared",
    "log_norm_vs_log_time_slope",
    "log_norm_vs_log_time_r_squared",
...
def duration_case_tag(model_scope: str, step_days: float, duration_years: float, renorm_years: float, perturbation_m: float) -> str:
    return (
        "two_body_tangent_duration_"
        f"{model_scope}_dur{tag_float(duration_years)}yr_"
...
def build_duration_row(
    *,
    ...
    validation = read_first_csv_row(run_output_dir / f"two_body_validation_{tag}.csv")
...
def write_scaling_outputs(rows: list[dict]) -> list[dict]:
    ...
    for key, group_rows in sorted(grouped.items()):
        model_scope, step_days, renorm_years, perturbation_m = key
        finite_durations = [
...
def write_plots(rows: list[dict]) -> None:
    ...
    for model_scope in sorted({row["model_scope"] for row in rows}):
        subset = [
            row for row in rows
            if row["model_scope"] == model_scope
            and math.isfinite(safe_float(row["lambda_1_per_year"]))
        ]
        if not subset:
            continue

        fig, ax = plt.subplots(figsize=(9, 5))
        for key in sorted({(row["step_days"], row["renorm_years"], row["perturbation_m"]) for row in subset}):
            group = [row for row in subset if (row["step_days"], row["renorm_years"], row["perturbation_m"]) == key]
            group.sort(key=lambda row: safe_float(row["duration_years"]))
            ax.plot(
                [safe_float(row["duration_years"]) for row in group],
                [safe_float(row["lambda_1_per_year"]) for row in group],
                marker="o",
                linewidth=0.8,
                markersize=2.5,
                alpha=0.65,
        ...
        ax.set_ylabel("finite-time lambda [1/year]")
        ax.set_title(f"Finite-time lambda vs duration: {model_scope}")
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        fig.savefig(output_base / f"finite_time_lambda_vs_duration_{model_scope}.png", dpi=150)
        plt.close(fig)
        ...
        for key in sorted({(row["step_days"], row["renorm_years"], row["perturbation_m"]) for row in subset}):
            ...
            ax.plot(
                [safe_float(row["duration_years"]) for row in group],
                [safe_float(row["accumulated_log_growth_final"]) for row in group],
                marker="o",
                linewidth=0.8,
                markersize=2.5,
                alpha=0.65,
        ...
        ax.set_title(f"Accumulated log growth vs duration: {model_scope}")
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        fig.savefig(output_base / f"accumulated_log_growth_vs_duration_{model_scope}.png", dpi=150)
        plt.close(fig)


def build_no_renorm_row(model_scope: str, step_days: float, duration_years: float, perturbation_m: float, tag: str, returncode: int) -> dict:
    row = {name: "" for name in no_renorm_fields}
    row.update(
        {
            "model_scope": model_scope,
            "step_days": step_days,
    ...
    lyap_summary = read_json(run_output_dir / f"lyapunov_summary_{tag}.json")
    summary = read_json(run_output_dir / f"summary_{tag}.json")
    fits = lyap_summary.get("no_renormalization_fits") or {}
    time_fit = fits.get("log_separation_vs_time", {})
    log_fit = fits.get("log_separation_vs_log_time", {})
    time_r2 = safe_float(time_fit.get("r_squared"))
    log_r2 = safe_float(log_fit.get("r_squared"))
    warnings = list(lyap_summary.get("warnings", []))
    row.update(
        {
            "log_norm_vs_time_slope": finite_or_blank(time_fit.get("slope")),
            "log_norm_vs_time_r_squared": finite_or_blank(time_r2),
            "log_norm_vs_log_time_slope": finite_or_blank(log_fit.get("slope")),
            "log_norm_vs_log_time_r_squared": finite_or_blank(log_r2),
            "preferred_growth_model": "log_time" if log_r2 > time_r2 else "linear_time",
            "runtime_seconds": finite_or_blank(summary.get("runtime", {}).get("wall_clock_seconds")),
            "warning_count": len(warnings),
            "main_warnings": "; ".join(warnings[:4]),
        }
...
with ladder_csv.open("w", newline="") as matrix_file:
    writer = csv.DictWriter(matrix_file, fieldnames=ladder_fields)
    writer.writeheader()

    for model_scope, config in base_configs.items():
        for step_days in config["steps"]:
            for renorm_years in config["renorms"]:
                for perturbation_m in config["perturbations"]:
                    for duration_years in config["durations"]:
                        case_count += 1
                        if max_cases is not None and case_count > max_cases:
                            break
                        tag = duration_case_tag(
                            model_scope,
                            step_days,
                            duration_years,
                            renorm_years,
                        ...
                        returncode = 0
                        if not (skip_existing and summary_path.exists() and validation_path.exists()):
                            cmd = [
                                python_bin,
                                "-m",
                                ...
                                f"{max(renorm_years, 0.1 * duration_years):g}",
                            ...
                            print(
                                f"[{case_count:03d}] {model_scope} duration={duration_years:g} yr "
                                f"step={step_days:g} d renorm={renorm_years:g} yr pert={perturbation_m:g} m",
                                flush=True,
                            )
                            returncode = run_case(cmd, run_output_dir / f"{tag}.log")
                        else:
                            print(f"[{case_count:03d}] reuse {tag}", flush=True)
...
summary_rows = write_scaling_outputs(rows)
write_plots(rows)

no_renorm_rows: list[dict] = []
with no_renorm_csv.open("w", newline="") as file_obj:
    writer = csv.DictWriter(file_obj, fieldnames=no_renorm_fields)
    writer.writeheader()
    if run_no_renorm:
        case_index = 0
        for model_scope, body in [("two_body_jupiter", "jupiter"), ("two_body_saturn", "saturn")]:
            for step_days in [1.0, 4.0]:
                for perturbation_m in [1.0, 1000.0]:
                    case_index += 1
                    tag = (
                        "tangent_no_renorm_duration_"
                        f"{model_scope}_step{tag_float(step_days)}d_"
                        f"pert{tag_float(perturbation_m)}m_10000yr"
                    )
                    summary_path = run_output_dir / f"lyapunov_summary_{tag}.json"
                    returncode = 0
                    if not (skip_existing and summary_path.exists()):
                        ...
                        print(
                            f"[no-renorm {case_index:02d}] {model_scope} step={step_days:g} d pert={perturbation_m:g} m",
                            flush=True,
                        )
                        returncode = run_case(cmd, run_output_dir / f"{tag}.log")
                    else:
                        print(f"[no-renorm {case_index:02d}] reuse {tag}", flush=True)
                    row = build_no_renorm_row(model_scope, step_days, 10000.0, perturbation_m, tag, returncode)
                    no_renorm_rows.append(row)
                    writer.writerow(row)
                    file_obj.flush()
