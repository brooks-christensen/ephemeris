...
STEP_DAYS="${STEP_DAYS:-2}"
TAG="${TAG:-full_10kyr_baseline_step${STEP_DAYS}d}"
RESUME="${RESUME:-1}"
...
if [[ "${WITH_FREQUENCY}" == "1" ]]; then
  EXTRA_ARGS+=(--with-frequency-map --frequency-window-years 2500 --frequency-step-years 1000 --frequency-bodies all)
fi

echo "[Run] 10 kyr full reduced Solar System baseline"
echo "[Run] Broad conservation/secular survey; not a Mercury-sensitive Lyapunov run."
echo "[Run] step_days=${STEP_DAYS}; WITH_FREQUENCY=${WITH_FREQUENCY}"
